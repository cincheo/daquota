...
<?php
    ...
    if($_FILES["file"]["name"]) {
        ...
        if(!$continue) {
            ...
            // BACKUP
            if (!is_dir('sites_backup')) {
                mkdir('sites_backup', 0777);
            }
            createZip('sites_backup/'.$APP_NAME.'-snapshot-'.date('Y-m-d-H-i-s-u').'.zip', realpath('../..'));

            if (!is_dir('sites_upload')) {
                mkdir('sites_upload', 0777);
            }
            $targetZip = 'sites_upload/' . $filename; // target zip file
