<?php
    ...
    $authorized = $_POST['user'] == 'admin' && $_POST['password'] == $ADMIN_PASSWORD;
    if (!$authorized) {
        $file = $SYNC_DATA_DIR.'/admin/users.json';
        $users = json_decode(json_decode(file_get_contents($file), true)['data'], true);
        $index = array_search($_POST['user'], array_column($users, 'login'));
        if ($index !== false && $users[$index]['password'] == $_POST['password']) {
            $authorized = true;
            $user = $users[$index];
            unset($user['password']);
        }
